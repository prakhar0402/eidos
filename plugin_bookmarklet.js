javascript: (function() {
   ...
   if (!s || s == '') s = prompt('Enter search terms for Eidos', '');
   var x = decodeURI(encodeURIComponent(s)).split(" ")[0];
   var des = "Description returned by Eidos";
   var bs = "Some other bullshit returned.";
   SelRange = s.getRangeAt(0);
	...
	var render_definition = function(data){
		...
		createHandler = function(elem){
			return function(){elem.style.display='none';};
		};
